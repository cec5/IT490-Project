...
if ($targetAction == "pushBundle") {
    ...
    if (isset($returnedResponse["version"])) {
        ...
        $res = ssh2_exec($connection, "cd /opt/store; chmod ugo+x /opt/store/$bundleName-v$versionNumber.tar.gz; mkdir $bundleName-v$versionNumber; tar -xzvf /opt/store/$bundleName-v$versionNumber.tar.gz -C ./$bundleName-v$versionNumber; echo done; pwd; rm $bundleName-v$versionNumber.tar.gz; chmod ugo+rwx -R ..");
    ...
    $returnedResponse = sendToDeployment($request);
    echo var_dump($returnedResponse);
} else if ($targetAction == "listBundles") {

    $request = ["type" => "listBundles"];
    $returnedResponse = sendToDeployment($request);
    echo var_dump($returnedResponse);
} else {
    echo "Available commands:\n\tpushBundle [BUNDLE_NAME]\n\tapproveBundle [BUNDLE_NAME] [VERSION] [PASS/FAIL]\n\tfanout [BUNDLE_NAME] [PROD|QA]\n\n";
