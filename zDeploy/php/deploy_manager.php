...
if ($targetAction == "pushBundle") {
    ...
    if (isset($returnedResponse["version"])) {
        ...
        $res = ssh2_exec($connection, "cd /opt/store; chmod ugo+x /opt/store/$bundleName-v$versionNumber.tar.gz; mkdir $bundleName-v$versionNumber; tar -xzvf /opt/store/$bundleName-v$versionNumber.tar.gz -C ./$bundleName-v$versionNumber; echo done; pwd; rm $bundleName-v$versionNumber.tar.gz; chmod ugo+x -R .");
